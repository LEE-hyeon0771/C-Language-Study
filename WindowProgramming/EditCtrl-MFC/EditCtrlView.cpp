...
void CEditCtrlView::OnInitialUpdate()
{
	...
	m_edit.ModifyStyle(ES_AUTOHSCROLL, ES_MULTILINE | ES_AUTOVSCROLL | WS_VSCROLL | WS_HSCROLL);

	CFile file;
	if (file.Open(_T("EditCtrl.vcxproj"), CFile::modeRead))
	{
		CString strText;
		CString strLine;
		...
		while (UINT bytesRead = file.Read(buffer, bufSize - 1))
		{
			buffer[bytesRead] = '\0'; 
			strLine = buffer;
			strText += strLine;
	...
	{
		MessageBox(_T("파일을 열 수 없습니다."), _T("오류"), MB_ICONERROR);
	}
